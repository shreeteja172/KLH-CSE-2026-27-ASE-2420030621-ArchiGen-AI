...
export type MermaidDiagramControls = {
  fit: () => void;
  full: () => void;
};

const MermaidDiagram = forwardRef<MermaidDiagramControls, Props>(
  function MermaidDiagram({ diagram }, ref) {
    ...
    useLayoutEffect(() => {
      ...
      const updateLines = () => {
        ...
        const nextLines = diagram.relationships.flatMap(
          (relationship, index) => {
      ...
      const observer = new ResizeObserver(() => {
        ...
        const boundedFit = Math.max(
          25,
          Math.min(100, Math.round(nextFit * 100)),
        );
